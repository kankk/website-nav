export default [{
  title: 'AlloyTeam',
  link: 'http://www.alloyteam.com/',
  description: '腾讯Web前端团队',
  icon: 'https://nav.mangokk.me/icons/alloyteam.jpg',
}, {
  title: 'FED',
  link: 'http://taobaofed.org/',
  description: '内容涵盖Web和Node',
  icon: 'https://nav.mangokk.me/icons/fed.png'
}, {
  title: '奇舞团',
  link: 'https://75team.com/',
  description: '奇虎360前端开发团队',
  icon: 'https://nav.mangokk.me/icons/75team.png',
}, {
  title: '凹凸实验室',
  link: 'https://aotu.io/',
  description: '京东用户体验设计部',
  icon: 'https://nav.mangokk.me/icons/aotu.png',
}, {
  title: '掘金',
  link: 'https://juejin.im/',
  description: '中文技术社区',
  icon: 'https://nav.mangokk.me/icons/gold.png',
}, {
  title: 'CNode',
  link: 'https://cnodejs.org/',
  description: '最大Node.js中文社区',
  icon: ''
}, {
  title: 'GitHub',
  link: 'https://github.com/',
  description: '开源/私有软件项目的Git托管平台',
  icon: 'https://nav.mangokk.me/icons/github.png',
}, {
  title: 'Vue中文论坛',
  link: 'https://forum.vuejs.org/c/chinese',
  description: 'Vue官方中文论坛',
  icon: '',
}, {
  title: 'React China',
  link: 'http://react-china.org/',
  description: '',
  icon: 'https://nav.mangokk.me/icons/reactchina.png',
}, {
  title: 'V2EX',
  link: 'https://www.v2ex.com/',
  description: '一个汇集各类奇妙好玩的话题和流行动向的网站',
  icon: ''
}, {
  title: 'W3cplus',
  link: 'http://www.w3cplus.com/',
  description: '推广前端行业技术',
  icon: 'https://nav.mangokk.me/icons/w3cplus.png'
}, {
  title: 'Awesomes.cn',
  link: 'https://www.awesomes.cn/',
  description: '国人维护的前端资源库, 深度对接到Github, 分类和展现清晰',
  icon: 'https://nav.mangokk.me/icons/awesomes.png'
}, {
  title: 'Vue (中文)',
  link: 'https://cn.vuejs.org/',
  description: '构建数据驱动的web界面的渐进式框架',
  icon: 'https://nav.mangokk.me/icons/vue.png'
}, {
  title: 'React (英文)',
  link: 'https://facebook.github.io/react/',
  description: '用于构建用户界面的JavaScript库',
  icon: 'https://nav.mangokk.me/icons/react.png'
}, {
  title: 'React (中文)',
  link: 'https://doc.react-china.org/',
  description: '用于构建用户界面的JavaScript库',
  icon: 'https://nav.mangokk.me/icons/react.png'
}, {
  title: 'React-Native',
  link: 'https://reactnative.cn/',
  description: '使用React构建原生app的框架',
  icon: 'https://nav.mangokk.me/icons/react.png'
}, {
  title: 'Element',
  link: 'http://element.eleme.io/2.0/#/zh-CN/',
  description: '饿了么前端团队出品的基于Vue的UI组件库',
  icon: ''
}, {
  title: 'Ant Design',
  link: 'https://ant.design/index-cn',
  description: '蚂蚁金服前端团队出品的基于React的UI组件库',
  icon: ''
}, {
  title: 'AntV',
  link: 'http://antv.alipay.com/zh-cn/index.html',
  description: '蚂蚁金服前端团队出品的数据可视化解决方案',
  icon: ''
}, {
  title: 'Moment.js',
  link: 'http://momentjs.cn/',
  description: '用于日期格式化处理的JavaScript类库',
  icon: ''
}, {
  title: 'Numeral.js',
  link: 'http://numeraljs.com/',
  description: '用于格式化和数字四则运算的JavaScript类库',
  icon: ''
}, {
  title: 'Lavas',
  link: 'https://lavas.baidu.com/',
  description: '百度出品的基于Vue的PWA解决方案, 帮助开发者快速搭建PWA应用, 解决接入PWA的各种问题',
  icon: ''
}, {
  title: 'ECMAScript6 教程',
  link: 'http://es6.ruanyifeng.com/',
  description: '阮一峰出品老师的ECMAScript6教程',
  icon: 'https://nav.mangokk.me/icons/ecmascript6.png'
}, {
  title: 'JavaScript 标准参考教程',
  link: 'http://javascript.ruanyifeng.com/',
  description: '阮一峰出品老师的JavaScript教程',
  icon: 'https://nav.mangokk.me/icons/javascript.png'
}, {
  title: 'MDN web doc',
  link: 'https://developer.mozilla.org/zh-CN/',
  description: '',
  icon: 'https://nav.mangokk.me/icons/mdn.png'
}, {
  title: 'Vue',
  link: 'https://cn.vuejs.org/v2/guide/',
  description: '官网指南',
  icon: 'https://nav.mangokk.me/icons/vue.png'
}, {
  title: 'Vue 风格指南',
  link: 'https://cn.vuejs.org/v2/style-guide/',
  description: '官方代码风格指南',
  icon: 'https://nav.mangokk.me/icons/vue.png'
}, {
  title: 'vue-router',
  link: 'https://router.vuejs.org/zh-cn/',
  description: '',
  icon: 'https://nav.mangokk.me/icons/vue.png'
}, {
  title: 'Vuex',
  link: 'https://vuex.vuejs.org/zh-cn/',
  description: '',
  icon: 'https://nav.mangokk.me/icons/vue.png'
}, {
  title: 'react-router',
  link: 'http://www.uprogrammer.cn/react-router-cn/index.html',
  description: '',
  icon: 'https://nav.mangokk.me/icons/react.png'
}, {
  title: 'Redux',
  link: 'http://cn.redux.js.org/index.html',
  description: '',
  icon: 'https://nav.mangokk.me/icons/redux.png'
}, {
  title: 'jQuery 中文文档(非官方)',
  link: 'http://jquery.cuishifeng.cn/',
  description: '',
  icon: 'https://nav.mangokk.me/icons/jquery.png'
}, {
  title: 'TypeScript中文文档',
  link: 'https://www.tslang.cn/index.html/',
  description: '官方网站(http://www.typescriptlang.org/)的中文翻译',
  icon: ''
}, {
  title: 'ES^ Katas',
  link: 'http://es6katas.org/',
  description: '通过单元测试的方式学习ES6, 强烈推荐',
  icon: ''
}, {
  title: 'React-Router Training',
  link: 'https://reacttraining.com/react-router/',
  description: '英文文档, 学习RR4的优秀文档',
  icon: 'https://nav.mangokk.me/icons/react-router-train.png'
}, {
  title: 'React小书',
  link: 'http://huziketang.com/books/react/',
  description: '',
  icon: ''
}, {
  title: 'Express',
  link: 'http://www.expressjs.com.cn/',
  description: '基于Node.js的Web开发框架',
  icon: ''
}, {
  title: '如何通过饿了么 Node.js 面试',
  link: 'https://elemefe.github.io/node-interview/#/sections/zh-cn/',
  description: 'Node教程',
  icon: ''
}, {
  title: 'Sequelize.js',
  link: 'http://docs.sequelizejs.com/',
  description: 'Node.js的ORM库',
  icon: 'https://nav.mangokk.me/icons/sequelizejs.png'
}, {
  title: 'Egg.js',
  link: 'http://eggjs.org/zh-cn/',
  description: 'Egg.js 为企业级框架和应用而生',
  icon: ''
}, {
  title: 'Webpack',
  link: 'https://doc.webpack-china.org/',
  description: '代码模块化构建打包工具',
  icon: 'https://nav.mangokk.me/icons/webpack.png'
}, {
  title: 'Babel',
  link: 'https://babeljs.io/',
  description: '使用最新的规范来编写js',
  icon: 'https://nav.mangokk.me/icons/babel.png'
}, {
  title: 'Regexper',
  link: 'https://regexper.com/',
  description: '可视化正则表达式',
  icon: ''
}, {
  title: 'CodePen',
  link: 'http://codepen.io/',
  description: '在线代码测试工具',
  icon: 'https://nav.mangokk.me/icons/codepen.png'
}, {
  title: '张鑫旭',
  link: 'http://www.zhangxinxu.com/wordpress/',
  description: 'CSS, 阅文集团',
  icon: ''
}, {
  title: '阮一峰',
  link: 'http://www.ruanyifeng.com/blog/',
  description: '',
  icon: ''
}, {
  title: 'Iconfont',
  link: 'http://iconfont.cn/',
  description: '阿里巴巴矢量图标库',
  icon: ''
}]
